...
class Model(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions):
        ...
        self.optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=0.01)  # Add L2 regularization
...
def run(train=True, model_name="models", epochs=50, steps=500):
    ...
    for e in range(epochs):
        ...
        print(f"epoch: {e}")
        print(f"Exploring: {brain.ratio[1]} Exploiting: {brain.ratio[0]}")
        print(f"Epsilon: {brain.epsilon}")
        print(f"Times decreased: {brain.no_times_dec}")
        select_lane = [
